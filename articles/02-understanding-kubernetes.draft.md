...
>
> 초안 단계입니다. 앞부분은 기존 연재 원고의 Kubernetes 개요를, 뒷부분은 `Introduction/AboutKubernetes.md`의 리소스·구성 요소·설계 철학을 붙인 형태입니다. 문체 통일·중복 정리·이미지 경로 정리는 다음 작업에서 다듬습니다.
...
그렇다고 배우기 쉽다는 뜻은 아닙니다. 처음에는 구성 요소와 설정 파일이 많아 “왜 이렇게 복잡하지?”가 먼저 나옵니다. 조금 익숙해지면 “이런 것까지 되는구나” 쪽으로 질문이 바뀌는 경우가 많습니다. 이 글도 같은 순서를 따릅니다. 곧바로 모든 이름을 외우기보다, Kubernetes가 **어떤 문제를 정책으로 바꾸는지**부터 보고, 이어서 API·클러스터·원하는 상태라는 뼈대를 잡은 뒤, 핵심 리소스와 구성 요소·설계 철학까지 이어집니다.
...
흐름은 단순합니다. 실행에 필요한 설계도(Image)로 상자를 만들고, 원하는 상태를 적고, 클러스터가 그 차이를 줄이게 맡긴다. 아래부터는 그 도시를 **조금 더 구체적으로**—핵심 리소스, 구성 요소, 동작 방식, 설계 철학—까지 내려가 봅니다.

---

## (보강) AboutKubernetes — 등장 배경을 한 번 더 깊게

아래 절은 `Introduction/AboutKubernetes.md` §1을 초안에 옮긴 것입니다. 앞 절과 겹치는 비유·사례가 있어, 다듬을 때 **한쪽으로 흡수**하거나 **차이만 남기는** 편집이 필요합니다.
...
<p align="center">
  <img src="../images/Introduction/2.4_Kubernetes_Namespace.png" width="100%">
</p>
...
이전 글에서 손으로 느낀 Docker 운용의 빈칸과, 이번 글에서 잡은 API·Desired State·핵심 리소스·Control Plane/Worker·선언형 Control Loop를 이어 두면, 이후 주차의 실습이 “명령 암기”가 아니라 **선언을 맞추고 관찰하는 연습**으로 읽히기 시작합니다.

다음 글에서는 도시 운영을 실제로 돌리는 구성(창구 뒤의 역할들)이 원하는 상태를 어떻게 유지하는지로, 실습과 함께 더 깊게 들어갑니다.

---

<!-- draft: split from 01-from-vm-to-kubernetes.publish.md + Introduction/AboutKubernetes.md (part 2/2) -->
