...
> 15주 연재의 둘째 글입니다. 이번 주는 Kubernetes가 무엇인지, 왜 배우는지, 핵심 개념이 어떻게 이어지는지를 개략적으로 정리합니다.

## 들어가며

이전 글에서 물리 서버에서 VM, Container와 Docker, Compose와 Swarm으로 이어진 배경을 따라왔고, Docker만으로 운용할 때 사람이 직접 반복하는 작업을 시나리오로 확인했습니다. 이번 글은 그 한계를 바탕으로, Kubernetes가 왜 등장했는지, 무엇을 하는지, 실무에서 다루는 대상과 클러스터가 어떻게 맞물리는지를 개략적으로 정리합니다.
...
그렇다고 배우기 쉽다는 뜻은 아닙니다. 처음에는 이름과 설정 파일이 많아 “왜 이렇게 복잡하지?”가 먼저 나오기 쉽습니다. 조금 익숙해지면 “이런 것까지 되는구나” 쪽으로 질문이 바뀌는 경우가 많습니다. 이 글은 그 순서에 맞춰, 곧바로 모든 이름을 외우기보다 **필요성, 설계 철학, 핵심 개념, 선언이 Pod로 실행되기까지, 명령어로 살펴보기**를 따라갑니다.
...
선언형의 강점은 자동 복구만이 아닙니다. 운영 설정이 YAML 매니페스트로 남기 때문에 인프라와 배포를 코드처럼 다룰 수 있습니다. 변경을 Git 커밋으로 남기고, 파일만 있으면 같은 상태를 다시 만들 수 있습니다. 운영 변경을 리뷰하고, 이전 커밋으로 되돌려 매니페스트를 다시 적용할 수도 있습니다. 이런 방식을 **IaC**(Infrastructure as Code, 인프라를 코드로 관리), 더 나아가 Git 저장소를 기준으로 맞추는 **GitOps**로 이어 가기도 합니다.
...
![직접 호출과 Watch 비교](../images/articles/02/15-direct-vs-watch.svg)
...
매니페스트를 API에 제출하면, 플랫폼이 그 내용을 읽고 어느 노드에 워크로드를 올릴지 정합니다. 앞에서 본 제어 루프가 실제 상태와의 차이를 줄이려 합니다. 애플리케이션이 Node.js든 Go든, Kubernetes 입장에서는 컨테이너 이미지와 매니페스트가 중요합니다. 언어가 달라도 같은 API와 매니페스트 방식으로 운영을 요청할 수 있다는 점이, 팀 규모가 커질 때 특히 도움이 됩니다.
...
클러스터는 크게 **Worker Node**(워커 노드, 실제 워크로드가 실행되는 서버)와 **Control Plane**(컨트롤 플레인, 클러스터를 제어하는 구성 요소)으로 나뉩니다.
...
Worker Node는 실제 워크로드가 돌아가는 머신입니다. 노드마다 아래 프로세스가 함께 동작합니다.

**Kubelet**은 각 노드에서 돌아가는 **에이전트**입니다. “이 노드에서 실행해야 할 Pod”를 API에서 받아오고, 아래에서 볼 Container Runtime을 호출해 컨테이너를 만들고 시작하거나 중지합니다. Pod와 노드 상태는 다시 API 쪽으로 보고합니다.
...
Control Plane은 클러스터를 **제어하는 구성 요소**입니다. Desired State를 해석하고 유지하며, 워크로드가 어느 노드에 갈지 결정하는 등의 판단을 담당합니다. 대표적으로 API Server, Scheduler, Controller Manager, etcd가 여기서 동작합니다.
...
운영에서는 API와 상태 저장소가 **한곳에만** 있으면, 그 한곳이 멈출 때 클러스터 운영 자체가 흔들릴 수 있습니다. 그래서 **고가용성**(HA, High Availability, 일부가 죽어도 전체가 멈추지 않게 하는 구성)을 두는 경우가 많습니다. 직관만 잡으면 충분합니다. API Server를 여러 대 두고, 사용자는 **하나의 주소**로 요청을 보냅니다. 그중 일부가 죽어도 나머지 API Server가 요청을 이어받습니다. 상태를 담는 etcd도 보통 여러 대(흔히 3대처럼 홀수)로 둡니다. **과반**(쿼럼, quorum)이 남아 있으면 읽기와 쓰기를 이어갑니다. Worker Node는 그 아래 여러 대가 워크로드를 나눠 받습니다. Control Plane을 한 대가 아니라 여러 대가 이어받도록 구성한다고 보면 됩니다.
...
![멈춘 컨테이너 복구 비교](../images/articles/02/16-scenario-recovery.svg)
...
![컨테이너 개수 스케일 아웃 비교](../images/articles/02/17-scenario-scale.svg)
...
| **한계점:** 포트 충돌을 사람이 피해야 하고, 로드 밸런서 upstream도 같이 수정                                                                                           | **개선점:** Pod 복제본 개수만 선언하면 되고, 포트 목록을 외울 필요가 없음                            |
...
![컨테이너 이미지 버전 업데이트 비교](../images/articles/02/18-scenario-update.svg)
...
![요청 부하 분산과 서비스 이름 찾기 비교](../images/articles/02/19-scenario-service.svg)
...
| Docker만 사용                                                                      | Kubernetes                                                                       |
| ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| upstream에 `host:8080` … `host:8083` 나열 후 재시작 `docker inspect`로 IP를 확인해 앱 설정에 기입 | `kubectl expose deployment web --port=80 --type=ClusterIP` `kubectl get svc web` |
...
![배포한 컨테이너와 진입점을 정리하며 지우기 비교](../images/articles/02/20-scenario-cleanup.svg)
