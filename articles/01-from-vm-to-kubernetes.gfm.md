...
그렇다고 배우기 쉽다는 뜻은 아닙니다. 처음에는 구성 요소와 설정 파일이 많아 “왜 이렇게 복잡하지?”가 먼저 나옵니다. 조금 익숙해지면 “이런 것까지 되는구나” 쪽으로 질문이 바뀌는 경우가 많습니다. 이 연재의 앞부분도 같은 순서를 따릅니다. 곧바로 모든 이름을 외우기보다, Kubernetes가 **어떤 문제를 정책으로 바꾸는지**부터 보고, 이어서 API·클러스터·원하는 상태라는 뼈대를 잡습니다.
...
컨테이너를 여러 개 실행하는 것 자체는 어렵지 않습니다. 어려운 것은 운영 판단입니다. 트래픽이 늘었을 때 Docker Swarm에서는 관리자가 `docker service scale`처럼 직접 복제본 수를 늘는 식의 개입이 흔합니다. 언제 늘릴지, 몇 개까지 늘릴지, 언제 줄일지를 사람이 결정합니다. Kubernetes에서는 CPU 사용률 같은 조건을 정책으로 남겨 두고, 그 이후의 판단과 실행을 플랫폼에 맡기는 쪽으로 문제를 바꿉니다.

장애도 마찬가지입니다. 새벽 세 시에 컨테이너 하나가 죽었을 때 Docker만 있다면 `docker ps`로 확인하고 로그를 본 뒤 `docker restart`를 사람이 수행해야 합니다. Kubernetes에서는 원하는 개수와 실제 개수의 차이를 계속 맞춰 가는 장치가 있고, 어느 서버에 올릴지 고른 뒤 그 서버에서 컨테이너를 기동하는 흐름이 이어집니다. 사람이 개입하지 않아도 복구가 시작된다는 점이, “집을 짓는 공구”와 “도시를 운영하는 시스템”의 차이를 실감하게 합니다.

비유를 한 번 더 쓰면 이렇습니다. Docker는 잘 포장된 상자를 만들고 여는 일에 강하고, Docker Compose는 한 집 안의 상자들을 한 도면으로 정리하며, Docker Swarm은 작은 단지 규모의 분산을 돕고, Kubernetes는 규모가 커질 때 필요한 정책과 자동화를 표준에 가깝게 묶어 둔 플랫폼입니다. 다만 “플랫폼”이라는 말을 제대로 이해하려면, Docker Swarm과의 차이만으로는 부족합니다. Kubernetes가 실제로 무엇을 묶어서 제공하는지, 사용자가 어디에 원하는 상태를 적는지를 조금 더 구체적으로 볼 필요가 있습니다.
...
Docker만 있을 때 상자가 죽으면 사람이 `docker start`를 눌러 현장을 도면에 맞췄습니다. Kubernetes에서는 플랫폼이 **원하는 상태**와 **현장에 실제로 있는 상태**를 비교하고, 어긋나면 상자를 더 띄우거나 줄이거나 다른 건물로 옮기며 차이를 줄입니다. 단지 관리사무소가 “매장에 바리스타가 두 명이어야 한다”는 규칙을 받아 두고, 한 명이 빠지면 다시 채우는 일과 같습니다. 이 “차이를 계속 맞추는 흐름”을 이후 글에서는 **제어 루프(Control Loop)** 라고 부를 예정입니다. 오늘은 이름보다 **사람이 버튼을 누르던 일이 도시의 기본 업무가 된다**는 감각만 가져가면 됩니다.
...
배우는 첫 환경은 꼭 거대한 도시일 필요도 없습니다. 건물 한 채(단일 노드)로도 창구와 원하는 상태의 감각을 익힐 수 있습니다. 다만 그 감각은 설명만으로는 잘 잡히지 않습니다. **Docker로 상자를 직접 운용해 보는 과정**이 먼저 있어야 Kubernetes가 왜 필요한지, 무엇을 대신해 주는지가 몸으로 이해됩니다. 공구로 못을 박아 본 뒤에야 도시 운영 체계의 말이 선명해지는 것과 같습니다.

그래서 이번 주 실습은 Kubernetes YAML보다 한 걸음 앞에서, Docker로 컨테이너를 띄우고·멈추고·다시 살리고·늘리는 운용을 손으로 겪습니다. **원하는 상태를 사람이 맞출 때 무엇이 힘든지**를 느껴 두면, 다음 주에 도시의 제어 루프 이야기가 공허하지 않습니다.

흐름은 단순합니다. 실행에 필요한 설계도(Image)로 상자를 만들고, 원하는 상태를 적고, 클러스터가 그 차이를 줄이게 맡긴다. 이번 연재도 그 순서를 따라갑니다.
...
Kubernetes를 개념으로만 읽으면 “알아서 맞춰 준다”는 말이 쉽게 들립니다. 그 말을 제대로 이해하려면, **자동화가 없을 때 Docker 운용에서 사람이 무엇을 반복하는지**를 먼저 느껴 보는 편이 좋습니다. 아래 실습은 그 감각을 만들기 위한 것입니다. 목표는 명령을 외우는 것이 아니라, 상자가 죽거나 늘거나 바뀌었을 때 손이 가는 자리를 확인하는 일입니다. 각 시나리오 끝에서는 같은 문제가 Kubernetes에서 **어떤 방향으로 바뀌는지**만 한두 문장으로 연결합니다. 장치 이름은 이후 주차에서 붙입니다.
...
지금까지 겪은 일을 앞에서 본 Kubernetes 이야기와 맞춰 보면 흐름이 선명해집니다.
...
이번 주에 따라온 줄기는 이렇습니다. 자원을 나누려 VM이 등장했고, 환경을 가볍게 옮기려 Container와 Docker가 등장했으며, 컨테이너가 늘자 Docker Compose와 Docker Swarm이 필요해졌고, 운영 판단과 정책·생태계의 무게가 커지자 Kubernetes가 중심에 섰습니다. Kubernetes는 **API 창구로 원하는 상태를 받고**, 클러스터라는 도시에서 현장과의 차이를 줄이려 합니다. 손으로 반복한 `docker start`, 수동 스케일, upstream 수정, IP 지정, 볼륨 실수는, 다음 주에 원하는 상태와 제어 루프를 배울 때 “왜 플랫폼이 그 일을 맡는지”를 설명하는 감각이 됩니다.

다음 글에서는 도시 운영을 실제로 돌리는 구성(창구 뒤의 역할들)이 원하는 상태를 어떻게 유지하는지로 들어갑니다. 이름은 그 글에서 하나씩 풉니다.