...
이번 주에 따라온 줄기는 이렇습니다. 자원을 나누려 VM이 등장했고, 환경을 가볍게 옮기려 Container와 Docker가 등장했으며, 컨테이너가 늘자 Docker Compose와 Docker Swarm이 필요해졌습니다. 손으로 반복한 `docker start`, 수동 스케일, upstream 수정, IP 지정, 볼륨 실수는, **다음 글에서 Kubernetes의 원하는 상태와 제어 루프를 배울 때** “왜 플랫폼이 그 일을 맡는지”를 설명하는 감각이 됩니다.

다음 글에서는 Kubernetes가 왜 이토록 크고 왜 배워야 하는지부터, API·클러스터·핵심 리소스·구성 요소·설계 철학까지를 한 번에 잡아 둡니다. 이름은 그 글에서 하나씩 풉니다.
