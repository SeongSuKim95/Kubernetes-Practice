#!/usr/bin/env bash
# RoleBindingLab 로컬 사전 도구 검증.
#   필요한 도구: aws CLI v2, eksctl, kubectl, envsubst(gettext)
#   (이 랩은 helm 을 쓰지 않습니다 — ArgoCD/metrics-server 는 kubectl apply.)
#
# 사용:
...
REQUIRED_TOOLS=(aws eksctl kubectl envsubst)
...
if [[ -f "${TEMPLATE}" ]] && command -v envsubst >/dev/null 2>&1; then
  rendered="$(AWS_REGION=ap-northeast-2 CLUSTER_NAME=payflow-lab K8S_VERSION=1.33 \
     NODE_INSTANCE_TYPE=t3.medium envsubst <"${TEMPLATE}")"
  if echo "${rendered}" | grep -q 'name: payflow-lab' \
     && echo "${rendered}" | grep -q 'payflow.io/pool: payments'; then
