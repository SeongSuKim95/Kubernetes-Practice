...
```text
[Node A]  /tmp/my-app-cache  ←  Pod-1 이 캐시 파일 기록
     │
     │  Pod-1 삭제
     ▼
Case 1) Pod-2 가 Node A 에 다시 스케줄
        → 같은 /tmp/my-app-cache → 캐시 유지 

Case 2) Pod-2 가 Node B 에 스케줄 (다른 노드)
        → Node B 의 /tmp/my-app-cache (비어 있거나 다른 내용)
        → Node A 에 남은 캐시는 Pod-2 에서 보이지 않음 
```
