...
이 문서는 **(1) Kubernetes 네트워크 기본 구조 → (2) CNI 개념·동작 → (3) CNI 선택·설치 → (4) NetworkPolicy 집행 연계** 순서로 읽으면 됩니다.
...
> 11번(NetworkPolicy 문법·선택)을 먼저 보고 오면 "선언(11) → 집행(12)" 흐름이 자연스럽습니다.
...
컨테이너 오케스트레이션에서 네트워크는 4가지 문제로 나뉩니다. **CNI는 이 중 1·2번**을 책임집니다.

| # | 문제 | 누가 해결 |
|---|------|-----------|
| 1 | **컨테이너 ↔ 컨테이너** (같은 Pod) | `localhost` (Pod 내 netns 공유) |
| 2 | **Pod ↔ Pod** (같은/다른 노드) | **CNI** ← 이번 Lab |
...
> 기본은 **flat network(전부 연결)**. 그래서 CNI만 설치하면 Pod끼리 바로 통신됩니다. 막으려면 NetworkPolicy + **정책 집행 가능한 CNI**가 필요합니다.
