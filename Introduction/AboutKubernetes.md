...
이제 **Pod·Deployment·Service·Ingress**가 무엇인지 먼저 정리한 뒤, 그다음 장에서 클러스터를 이루는 **물리·프로세스 구성요소**(Control Plane, 노드, Kubelet 등)를 살펴봅니다.


## 2. Kubernetes의 핵심 개념 (Pod / Deployment / Service)
...
### 2.5 Pod / Deployment / Service / Ingress의 관계
...
앞 장에서 **Pod·Deployment·Service·Ingress**가 무엇인지(배포 단위) 살펴봤으니, 이번에는 그 객체들이 **어느 프로세스·노드에 의해** 실현되는지를 등장인물처럼 짚어보겠습니다.  
